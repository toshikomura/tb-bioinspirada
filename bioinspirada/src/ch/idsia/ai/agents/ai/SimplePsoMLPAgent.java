 ...
 * Time: 2:09:42 PM
 */
public class  SimplePsoMLPAgent implements Agent, Evolvable {

    private MLP mlp;
    private String name = "SimplePsoMLPAgent";
    final int numberOfOutputs = 6;
    final int numberOfInputs = 10;
    private MLP gbest;
    private MLP[] pbests;
    private MLP[][] particulas;
    private int[] scorepbest;
    private int scoregbest;

    public SimplePsoMLPAgent () {
        mlp = new MLP (numberOfInputs, 10, numberOfOutputs);
    }

    private SimplePsoMLPAgent (MLP mlp) {
        this.mlp = mlp;
    }

    public Evolvable getNewInstance() {
        return new SimplePsoMLPAgent(mlp.getNewInstance());
    }

    public Evolvable copy() {
        return new SimplePsoMLPAgent (mlp.copy ());
    }
